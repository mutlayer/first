...
<%
	...
	arr = (List<Map<String,Object>>)request.getAttribute("receive_deposit_his");
	...
	if(arr!=null){
		...
		for(int i=0;i<arr.size();i++){
			...
			if(arr.get(i).get("APPLY_MONEY")!=null){
				deposit_status =(String) arr.get(i).get("DEPOSIT_STATUS");
			}
			if(arr.get(i).get("UPDATEID")!=null){
				updateid =(String) arr.get(i).get("UPDATEID");
			}
			if(arr.get(i).get("DEPOSIT_BONUS")!=null){
				deposit_bonus =arr.get(i).get("DEPOSIT_BONUS").toString();
			}
			...
			obj.put("deposit_status",deposit_status);
			obj.put("updateid",updateid);
			obj.put("deposit_bonus",deposit_bonus);
		...
		//obj.put("list",list);
		receiveobj.put("receive_deposit_his",list);
	}else{
		receiveobj.put("receive_deposit_his",null);
	}
